...
import { Log, Page, PageRequest, DateRangeQuery } from '../..';
...
export class LogAPI {
  ...
  download(query: DateRangeQuery | null, clear: boolean): Observable<Blob> {
  ...
  getLogList(request: PageRequest<Log>, query: DateRangeQuery): Observable<Page<Log>> {
    let params = new HttpParams()
    ...
    if (query.start.length && query.end.length) {
      params = params
        .set('start', new Date(query.start).toISOString())
        .set('end', new Date(query.end).toISOString());
    }
